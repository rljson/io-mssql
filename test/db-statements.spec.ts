...
describe('DbStatements', () => {
  ...
  it('should generate correct CREATE DATABASE statement', () => {
    expect(dbStatements.createDatabase('testdb')).toBe(
  ...
  it('should generate correct rowCount SQL', () => {
    expect(dbStatements.rowCount('mytable')).toBe(
      'SELECT COUNT(*) AS totalCount FROM [myschema].[mytable_tbl]',
    );
  });

  it('should generate correct allData SQL', () => {
    expect(dbStatements.allData('mytable')).toBe(
      'SELECT * FROM [myschema].[mytable_tbl]',
    );
    expect(dbStatements.allData('mytable', 'foo_col,bar_col')).toBe(
      'SELECT foo_col,bar_col FROM [myschema].[mytable_tbl]',
    );
  });
  ...
  describe('MsSqlStatements.whereString', () => {
    it('should generate correct SQL for string value', () => {
      ...
      expect(dbStatements.whereString(whereClause)).toBe(" name_col = 'John'");
    });
    ...
    it('should generate correct SQL for boolean true value', () => {
      ...
      expect(dbStatements.whereString(whereClause)).toBe(' isActive_col = 1');
    });

    it('should generate correct SQL for boolean false value', () => {
      const whereClause: [string, boolean][] = [['isActive', false]];
      expect(dbStatements.whereString(whereClause)).toBe(' isActive_col = 0');
    });
  ...
  it('should generate correct joinExpression', () => {
    expect(dbStatements.joinExpression('mytable', 't')).toBe(
  ...
  it('should generate correct tableCfg SQL', () => {
    expect(dbStatements.tableCfg).toContain(
      'SELECT * FROM [myschema].tableCfgs_tbl WHERE key_col = ?',
    );
  });

  it('should generate correct tableCfgs SQL', () => {
    expect(dbStatements.tableCfgs).toBe(
      'SELECT * FROM [myschema].tableCfgs_tbl',
    );
  });

  it('should generate correct currentTableCfg SQL', () => {
    expect(dbStatements.currentTableCfg).toContain('WITH versions AS');
    expect(dbStatements.currentTableCfg).toContain(
  ...
  it('should generate correct insertTableCfg SQL', () => {
    ...
    if (
      ...
      Array.isArray(IoTools.tableCfgsTableCfg.columns)
    ) {
  ...
  it('should generate correct selection SQL', () => {
    const sql = dbStatements.selection(
      'mytable',
      'foo_col,bar_col',
      'foo_col = 1',
    );
  ...
  it('should generate correct alterTable SQL statements', () => {
    ...
    const result1 =
      'ALTER TABLE [myschema].[mytable_tbl] ADD foo_col NVARCHAR(MAX);';
